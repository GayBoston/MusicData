...
def get_playlist_track_ids(sp: spotipy.Spotify, playlist_id: str, max_retries: int = 5) -> list:
    ...
    while retries <= max_retries:
        try:
            results = sp.playlist_tracks(playlist_id, limit=100)
            tracks = results['items']
            track_ids.extend([item['track']['id'] for item in tracks if item['track'] and item['track']['id'] is not None])
...
def get_song_features_bulk(sp: spotipy.Spotify, track_ids: list, artist_genre_cache: dict, max_retries: int = 5) -> list:
    """
    Retrieves audio features, track information, and genres for a list of track IDs in bulk, handling 429 errors with retries.

    Args:
        sp (spotipy.Spotify): Authenticated Spotify client.
        track_ids (list): List of Spotify Track IDs.
        artist_genre_cache (dict): Cache to store artist genres to minimize API calls.
        max_retries (int): Maximum number of retry attempts for 429 errors.
    ...
    while retries <= max_retries:
        try:
            ...
            for track, features in zip(tracks_info, features_list):
                if track is None:
                    print(f"Track information not found. Skipping track ID {track}.", file=sys.stderr)
                    continue
                if features is None:
                    print(f"No audio features found for track ID {track['id']}. Skipping.", file=sys.stderr)
                    continue
                    ...
                    if artist_id:
                        if artist_id in artist_genre_cache:
                            ...
                            try:
                                ...
                                artist_genre_cache[artist_id] = artist_genres  # Cache the genres
                            except SpotifyException as e:
                                print(f"Spotify API error while fetching artist {artist_id}: {e}", file=sys.stderr)
                                artist_genres = []
                            except Exception as e:
                                print(f"Error fetching genres for artist {artist_id}: {e}", file=sys.stderr)
                                artist_genres = []
...
def process_playlist(sp: spotipy.Spotify, playlist_id: str, batch_size: int = 50) -> list:
    """
    Processes all tracks in a playlist in batches and retrieves their features and genres.
    ...
    # Initialize a cache for artist genres to minimize API calls
    artist_genre_cache = {}

    # Process in batches
    for i, batch in enumerate(chunkify(track_ids, batch_size), start=1):
        start_track = (i - 1) * batch_size + 1
        end_track = min(i * batch_size, total_tracks)
        print(f"Processing batch {i}: Tracks {start_track} to {end_track}")
        song_data = get_song_features_bulk(sp, batch, artist_genre_cache)
        song_data_list.extend(song_data)
        # Optional: Sleep between batches to further reduce the risk of rate limiting
        time.sleep(0.1)  # Adjust as needed
